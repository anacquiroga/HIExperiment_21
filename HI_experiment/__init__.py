...
import random, csv, math, itertools, time
...
def creating_session(subsession):    
    ...
    for p in subsession.get_players():
        print("Player Color " + str(i))
        p.color = next(colors)
        print("Player stimuli " + str(i) + ": " + datetime.now().strftime("%H:%M:%S"))
        i = i + 1
        stimuli = read_csv()
        for stim in stimuli:
            Question.create(player=p, **stim)
    global h
    print("Round " + str(h) + " of " + str(Constants.num_rounds) + " session creation done.")
    h = h + 1
# TREATMENTS (colors):
...
def calculate_payoff(player):
    ...
    payoff = math.ceil(points / convertion)
    if payoff > Constants.max_performance_pay:
        payoff = Constants.max_performance_pay
    elif payoff < 0:
        payoff = 0
...
class Instructions(Page):
    ...
    def vars_for_template(player):
        max_payout = Constants.max_performance_pay + Constants.show_fee
        return dict(
